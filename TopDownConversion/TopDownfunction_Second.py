...
def converttopdown(Hue, Saturation, Value, ROImask, cornermask, InputPic, OutputPic, Pic_size=(1920, 1080), plotcorner=False, plotline=False, automode=True):
    ...
    if(automode):
        ...
        #cv2.imshow('line_image_beforeS', line_image_before) #斜率合併之前所得到的線
        #cv2.waitKey()
        #合併斜率相似的線
        ...
        #cv2.imshow('line_image_afterS', line_image_after) #斜率合併後所得到的線
        #cv2.waitKey()
        ...
        #cv2.imshow('connectlineS', connectline) #斜率合併後所得到的線
        #cv2.waitKey()
    
    #cv2.imshow('x', connectline)
    #cv2.waitKey()        
    ...
    for i in range(1, cut):
        ...
        #cv2.line(OutputPic, (x, y), (xx, yy), (0, 0, 255))
    ...
    for i in range(1, cut):
        ...
        #cv2.line(OutputPic, (x, y), (xx, yy), (255, 0, 0))
    ...
    for i in range(-2, 3):
        ...
        cv2.circle(OutputPic, (int(x), int(y)), 5, (0, 0, 255), -1)
        cv2.circle(OutputPic, (int(xx), int(yy)), 5, (0, 0, 255), -1)
        cv2.line(OutputPic, (int(x), int(y)), (int(xx), int(yy)), (0, 255, 0), 1)
    ...
    line_mask = ROImask*0
    cv2.line(line_mask, (int(x), int(y)), (int(xx), int(yy)), 255)
    #cv2.imshow('greenline1', line_mask)
    ...
    line_mask = ROImask*0
    cv2.line(line_mask, (int(x), int(y)), (int(xx), int(yy)), 255)
    #cv2.imshow('greenline2', line_mask)
    #根據球洞位置判斷翻轉方向
